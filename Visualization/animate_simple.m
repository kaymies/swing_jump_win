function animate_simple(t,z,p, speed)
    ...
    axis([-.2 .2 -.2 1])
    h_ground = plot([-1 1],[0 0],'k-','LineWidth',1);
    hold on
    h_leg    = plot([0],[0],'-b',...
                'LineWidth',2,...
                'MarkerEdgeColor','r',...
    ...
    while toc < t(end)/speed                        % while there's real time left
        ...
        draw_lines(zint,p,h_spring,h_leg);
    end
    draw_lines(z(:,end),p,h_spring,h_leg);
end

function draw_lines(z,p,h_spring,h_leg)
    keypoints = keypoints_swing_jump_win(z,p);
    h_leg.XData = keypoints(1,[1,2,4:8]);
    h_leg.YData = keypoints(2,[1,2,4:8]);
